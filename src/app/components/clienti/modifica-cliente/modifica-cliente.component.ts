...
export class ModificaClienteComponent implements OnInit {
  ...
  loadUsers() {
    this.clientSrv.getbyId(this.id).subscribe((ris) => {
      ...
      const group: FormGroup = this.form.get(
        'indirizzoSedeOperativa'
      ) as FormGroup;
      group.controls['via'].patchValue(this.cliente.indirizzoSedeOperativa.via);
      group.controls['civico'].patchValue(
        this.cliente.indirizzoSedeOperativa.civico
      );
      group.controls['cap'].patchValue(this.cliente.indirizzoSedeOperativa.cap);
      group.controls['localita'].patchValue(
        this.cliente.indirizzoSedeOperativa.localita
      );
      const comuneGroup: FormGroup = group.get('comune') as FormGroup;
      const provinciaGroup: FormGroup = comuneGroup.get(
        'provincia'
      ) as FormGroup;
      comuneGroup.controls['id'].patchValue(
        this.cliente.indirizzoSedeOperativa.comune.id
      );
      provinciaGroup.controls['id'].patchValue(
        this.cliente.indirizzoSedeOperativa.comune.provincia.id
      );
    });
  }
